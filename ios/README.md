...
- **Local notifications + dose actions** ([`notification-scheduler/`](notification-scheduler/PildoraNotificationScheduler/), issue #49) — production `PildoraNotificationScheduler` package that schedules dose reminders as **local-only** notifications with actionable Taken/Snooze/Skip responses, rotating them within the iOS 64-notification limit (soonest-first, priority tiebreak), plus overdue badge + summary, rich content (name/dosage/instructions), onboarding permission handling, and automatic Apple Watch mirroring. Promotes the [`notification-spike`](notification-spike/) (#23) strategy into a tested, dependency-free slice: the only type touching `UserNotifications` sits behind a protocol seam, so the whole package (including a deterministic 48-hour / 20+ medication stress simulation) runs under `swift test` with no entitlements. On-device delivery testing remains gated on Apple Developer enrollment (#25). Documented in [`docs/ios-notifications.md`](../docs/ios-notifications.md).
...
- **Onboarding flow** ([`onboarding/`](onboarding/PildoraOnboarding/), issue #53) — production `PildoraOnboarding` package: the first-run experience that creates the master password (with a zxcvbn-style **strength meter** + explicit requirements), generates a **recovery key** and exports a printable **Recovery Kit PDF**, forces an explicit **unrecoverable-data** acknowledgement, creates the first vault, and offers **biometric unlock** and **iCloud Keychain backup** opt-ins plus a skippable **guided first medication**. Progress is persisted so the flow **resumes** after interruption (never persisting secret material — it resumes at the password step). The whole slice runs under `swift test` with no entitlements: crypto, keychain/biometrics, and persistence sit behind protocol seams (the app supplies the FFI-backed `OnboardingCrypto` that runs the real Argon2id→MUK→vault-key hierarchy and wipes key material via [`SecureBytes`](secure-memory/PildoraSecureMemory/) after wrapping). Built on the shared [design system](design-system/PildoraDesignSystem/) with VoiceOver + Dynamic Type. On-device biometric testing remains gated on Apple Developer enrollment (#25). Documented in [`docs/ios-onboarding.md`](../docs/ios-onboarding.md).
- **Xcode Rust build phase** ([`app/`](app/), issues #41 / #48 / #53) — Xcode app that cross-compiles `pildora-crypto-ffi` for the active SDK/arch via a Run Script build phase, so `Cmd+B` builds and runs with no manual step. Gates first launch on the [onboarding flow](onboarding/PildoraOnboarding/) (#53) — deriving the master key and storing the vault key in the Keychain (optionally behind biometrics / iCloud) — then hosts the medication UI ([`medication-list/`](medication-list/PildoraMedicationList/)) on the encrypted database via the crypto FFI + `VaultDatabaseManager`, with the FFI smoke test preserved behind a Diagnostics tab and XCUITests covering medication CRUD + autocomplete (#48). Generated with XcodeGen (`project.yml`).
